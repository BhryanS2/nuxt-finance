      ...
      <table class="mt-4 min-w-full divide-y divide-gray-200 shadow">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          >
            ...
            >
              <div class="w-72" v-if="category.isUpdate">
                <AppFormInput
                  v-model="category.name"
                  @keyup="updateCategory(category)"
                />
              </div>
              <template v-else>
                {{ category.name }}
            ...
            >
              <a
                ...
                @click.stop.prevent="toggleEditCategory(category)"
              >
                Edit
              </a>
...
export default {
  ...
  async asyncData({ store }) {
    const reponse = await store.dispatch("categories/getCategories");
    const data = reponse.map((category) => ({
      ...category,
      isUpdate: false,
    }));
  ...
  methods: {
    ...
    toggleEditCategory(category) {
      category.isUpdate = !category.isUpdate;
    },
